...
*/
/*    $Id: dmumps_io.h,v 1.13 2006/02/08 08:00:18 pamestoy Exp $ */
...
int dmumps_is_there_finished_request(int* flag,int* ierr);

int dmumps_clean_request(int* request_id,int* ierr);

int dmumps_test_request(int* request_id,int* flag,int* ierr);

int dmumps_wait_request(int* request_id,int* ierr);

int dmumps_wait_all_requests(int* ierr);

int dmumps_low_level_init_ooc_c(int* _myid, int* total_size_io,int* size_element,
			       int* async,char* dmumps_dir, char* dmumps_file,
			       int* dmumps_dim_dir, int* dmumps_dim_file,
			       int* ierr);

int dmumps_low_level_write_ooc_c( const int * strat_IO, 
				 void * address_block,
				 int * block_size,
				 int * pos_in_file,
				 int * file_number,
...
int dmumps_low_level_read_ooc_c( const int * strat_IO, 
			        void * address_block,
				int * block_size,
				int * from_where,
				int * file_number,
...
int dmumps_low_level_direct_read(void * address_block,
				   int * block_size,
				   int * from_where,
				   int * file_number,
				   int * ierr);

int dmumps_clean_io_data_c(int* myid,int* ierr);

int dmumps_get_max_nb_req(int *max,int* ierr);

int dmumps_get_max_file_size(double * max_ooc_file_size);

int dmumps_ooc_get_nb_files(int* nb_files);

int dmumps_ooc_get_file_name(int* indice,char* name,int* length);

int dmumps_ooc_set_file_name(int* indice,char* name,int* length,int* ierr);

int dmumps_ooc_alloc_pointers(int* dim,int* ierr);

int dmumps_ooc_init_vars(int* myid_arg, int* nb_file_arg,
			int* size_element,int* async,
			char* dmumps_dir, char* dmumps_file,
			int* dmumps_dim_dir, int* dmumps_dim_file,
			int *ierr);

int dmumps_ooc_start_low_level(int* ierr);

int dmumps_ooc_print_stats();

int dmumps_ooc_remove_file(char *name,int* ierr);
