...
*/
/* $Id: smumps_distribution.c,v 1.9 2003/04/25 12:25:46 jylexcel Exp $ */
/******************************************************************
 *   distribution.c    
 *   Tools to compute the distribution of the tree on the processors
 *   (see comments of the routine "smumps_distri"
 *
...
void smumps_frootcout(pos,ne,nd,frere,fils,tcout,ncout,k50)
   MONint pos;
   MONint k50;
  ...
  if (ne[pos] != 0 ) 
    {
     ...
     for ( nb = 0 ; nb < ne[pos] ; nb++) 
        { 
         smumps_frootcout(posi,ne,nd,frere,fils,tcout,ncout,k50);
...
/* calcul du cout de l'arbre complet */
 ...
 void smumps_fcout(n,ne,nd,frere,fils,tcout,ncout,k50)
   MONint n,k50;
   MONint *ne,*nd,*frere,*fils;
    double *tcout,*ncout;
{
  ...
  for (pos =  1; pos < n+1 ; pos++)
      {
        if (frere[pos] ==(n+1)) 
          ...
          {
           /* noeud principal */
           if(frere[pos] == 0) smumps_frootcout(pos,ne,nd,frere,fils,tcout,ncout,k50);
          }
      } 
...
/************/
liste *smumps_creerliste(cout,no)
  double cout;
 MONint no;
{
  liste *list0;
...
/************/
void smumps_ajouter(cout,no,list)  /* ajoute i a la liste non vide */
   double cout;
  MONint no;             
   liste *list;
{
 while(list->suivant) list = list->suivant;
 list->suivant = smumps_creerliste(cout,no);
}
/************/
liste *smumps_ajouter_avant(cout,no,list)  /* ajoute i a la liste non vide */
   double cout;
  MONint  no;             
   liste *list;
{
  liste *list1;
  list1 = smumps_creerliste(cout,no);
  list1->suivant = list;
  return list1;
}
...
/**************************************************/
liste *smumps_remplacer_aux(cout,list,list1,list2)
  double cout;
  liste *list,*list1,*list2;
{
 if (list->lcout==cout)
     ...
     {
      list->suivant = smumps_remplacer_aux(cout,list->suivant,list1,list2);
      return list;
     }
}
/************/
 liste *smumps_remplacer(cout,list,list1)
  double cout;
  liste *list;
  liste *list1;
{
...
/*list2 = (liste *)malloc(sizeof(liste));*/
  ...
  list = smumps_remplacer_aux(cout,list,list1,list2);
  return list;
}
/**********************************************************************/
...
/******************************/
  void smumps_classer(tab1,tab2,nb)
    double *tab1;
   MONint *tab2;
   MONint nb;
 {
...
/************/
 MONint smumps_mintab(tabcout,nb)
  double *tabcout;
  MONint nb;
{
  MONint pos,i;
...
/*  fait de maniere grossiere */
  ...
  void smumps_rangersimp(list,n,nbproc,procnode1,tabcout,poids)
    liste *list;
   MONint nbproc,n;
    double *tabcout;
   MONint *procnode1;
  ...
  MONint i,mini; 
...
#if defined(check)
   printf(" Warning smumps_rangersimp: simple strate0 mapping \n");
#endif
   ...
   if (nb<=nbproc) 
   ...
   else
    {
     for (i=1;i<=nb;i++)
     {
      mini = smumps_mintab(tabcout,nbproc);
...
/* rangement par groupement des noeuds les plus proches sur un meme processeur*/
  void smumps_ranger(list,n,nbproc,procnode1,tabcout,poids)
    liste *list;
    MONint nbproc,n;
    double *tabcout;
    MONint *procnode1;
...
/* list1 = (liste *)malloc(sizeof(liste));*/
   list1 = list;
   cout = 0.0;
   /* printf("smumps_ranger: liste des couts : \n");*/
   while(list1)
    {
...
/* rangement d'une strate */
  void smumps_rangerstrat(list,n,nbproc,procnode1,tabcout,poids)
    liste *list;
    MONint nbproc,n;
    double *tabcout;
    MONint *procnode1;
...
/************/
 MONint smumps_equilibrage(tabcout,nbproc)
    double *tabcout;
    MONint nbproc;
...
/************************************************/
  ...
  void smumps_sousarbreaux(procnode1,noeud,noproc,frere,fils)
 ...
 
 {
   ...
   if (noeud)
   {
     ...
     while (noeud >0 )
      {
       smumps_sousarbreaux(procnode1,noeud,noproc,frere,fils); 
       noeud = frere[noeud];
      }
   }
  }
...
/************/
  ...
  void smumps_sousarbre(list,procnode1,frere,fils)
   liste *list;
   MONint *procnode1,*frere,*fils;
...
/**************************************/
   void smumps_strateinit(list,procnode1,n,nbproc,nbroot,ne,frere,fils,tcout,tabcout,
                   poids)
...
/* rangement initial */
  if (nbroot>=nbproc)
    {
     smumps_ranger(*list,n,nbproc,procnode1,tabcout,poids);
     if (smumps_equilibrage(tabcout,nbproc)) fin=TRUE;
    }
...
/* remplacement du noeud de plus gros cout par ses fils */
   ...
   if (noeud)
      {
       noeud = - noeud;
       list1 = smumps_creerliste(tcout[noeud],noeud);
       /* printf("Noeud %d eclate en %d \n",pere,ne[pere]);*/
       for (i=2;i<(ne[pere]+1);i++)
        {
         noeud = frere[noeud];
         list1 = smumps_ajouter_avant(tcout[noeud],noeud,list1);
        }
       *list=smumps_remplacer(cout,*list,list1);
       smumps_ranger(*list,n,nbproc,procnode1,tabcout,poids);
/* test de l'equilibrage de la charge */
       if (smumps_equilibrage(tabcout,nbproc))
           {
              fin=TRUE ;
...
/* November 1999 : we keep the previous mapping 
       if (!smumps_equilibrage(tabcout,nbproc)) 
        {
         smumps_rangersimp(*list,n,nbproc,procnode1,tabcout,poids);
        }
*/
      }
  }

    smumps_sousarbre(*list,procnode1,frere,fils) ;

}

/**********************************************************************/
  MONint smumps_appartient(noeud,list)
   MONint noeud;
   liste *list;
{
  while(list)
...
/*************/
 ...
 MONint smumps_perevalideaux(noeud,nstk,frere)
  MONint noeud;
  MONint *nstk,*frere;
{
 ...
 if (noeud<0) return TRUE;
 else
  {
   ...
   else return smumps_perevalideaux(frere[noeud],nstk,frere);
  }
}
/************/
/* smumps_perevalide determine si on peut prendre un pere */
/*  un pere est valide si tous ses fils sont places */

 MONint smumps_perevalide(noeud,nstk,frere,fils)
  MONint noeud;
  MONint *nstk,*frere,*fils;
{
  while(noeud>0) noeud = fils[noeud];
  noeud = -noeud;
  return smumps_perevalideaux(noeud,nstk,frere);
}
...
/************************************/
   ...
   void smumps_strate(n,procnode1,list,nbproc,ncout,frere,fils,tabcout,nstk,poids)
    MONint n,nbproc;
...
/* ajout du noeud a la strate suivante si son pere est valide */
     if (noeud)
     {
      if (smumps_perevalide(noeud,nstk,frere,fils))
      {
       if (debut) 
        {
         list1=smumps_creerliste(ncout[noeud],noeud);
         debut=FALSE;
        }
       else
       {
         if(!smumps_appartient(noeud,list1)) 
           smumps_ajouter(ncout[noeud],noeud,list1);
       }
      }
     }
    }
/* rangement de la strate sur les processeurs */
   if (!debut) smumps_rangerstrat(list1,n,nbproc,procnode1,tabcout,poids) ;
   else 
    {  procnode1[0] = 1;
       list1 = NULL;
    }
...
/* creation de la liste */
 liste *smumps_rep_liste(list,procnode,frere,fils,ne)
 ...
 while (list2)
  {
   ...
   if (!debut) 
    {
     if(pere) {smumps_ajouter(0.0,0,list1); nb++;nb_vid++;}
    }
   debut = FALSE;
   if(pere)
   {
    ...
    if(noeud)
     {
      if (noeud>0) 
        {
         smumps_ajouter(procnode[noeud]+100.0,noeud,list1);
         nb++;
        }
      else
       {
        ...
        for (i=1;i<=ne[-noed];i++)
         {
          smumps_ajouter(1.0*procnode[noeud],noeud,list1);
          noeud = frere[noeud];
         }
       }
     }
...
#if defined(DEBUG)
/***********************/
 void smumps_represent_aux(pos,procnode,frere,fils,ne)
 MONint *procnode,*frere,*fils,*ne;
{
 liste *list;
 list = smumps_creerliste(1.0,0);
 smumps_ajouter(1.0*procnode[pos],pos,list);
 while(list->lcout)
  {
    smumps_aff_liste(list);
    list = smumps_rep_liste(list,procnode,frere,fils,ne);
  }
}
/***********************/
 void smumps_represent(n,procnode,frere,fils,ne)
 MONint *procnode,*frere,*fils,*ne;
 MONint n;
{
 MONint i;
 for (i=1;i<=n;i++)
  {
   if (frere[i] == 0) 
     {
      smumps_represent_aux(i,procnode,frere,fils,ne);
      printf("\n");
     }
  }
}
...
void smumps_distri_(pn,pnbproc,ne,na,nd,frere,fils,procnode,
...
/* calcul des couts des noeuds de l'arbre */
       smumps_fcout(n,ne,nd,frere,fils,tcout,ncout,k50);
...
/* creation de la liste initiale des noeuds racines */
       for (i= 1;i<(n+1);i++)
           if (frere[i] == 0) 
            {
              if (debut) {
                          list  = smumps_creerliste(tcout[i],i);
                          list1 =list;
                          debut = FALSE;
                         } 
              else {
                   list1->suivant = smumps_creerliste(tcout[i],i);
                   list1          = list1->suivant;
                   }
            } 

/* calcul de la strate initiale */
       smumps_strateinit(&list,procnode1,n,nbproc,nbroot,ne,frere,fils,tcout,
                  tabcout,poids);
...
/* calcul des autres strates */
       ...
       while(procnode1[0]==0)
       {
         smumps_strate(n,procnode1,&list,nbproc,ncout,frere,fils,tabcout,nstk,poids);
         list1 = list;
         while(list1)
          {
...
#endif
#if defined(DEBUG)
    smumps_represent(n,procnode1,frere,fils,ne);                     
#endif
/****************************************************************/
...
void smumps_distri(pn,pnbproc,ne,na,nd,frere,fils,procnode,poids,ssarbre,nbsa,pk50)
    MONint *ne,*na,*nd,*frere,*fils,*poids,*nbsa,*pk50;
    MONint *pn,*pnbproc;
    MONint *procnode;
    MONint *ssarbre;
{
    smumps_distri_(pn,pnbproc,ne,na,nd,frere,fils,procnode,
             poids,ssarbre,nbsa,pk50);
}
...
void smumps_distri__(pn,pnbproc,ne,na,nd,frere,fils,procnode,poids,ssarbre,nbsa,pk50)
    MONint *ne,*na,*nd,*frere,*fils,*poids,*nbsa,*pk50;
    MONint *pn,*pnbproc;
    MONint *procnode;
    MONint *ssarbre;
