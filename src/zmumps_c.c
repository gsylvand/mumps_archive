...
*/
/* $Id: zmumps_c.c,v 1.16 2003/06/30 16:31:49 jylexcel Exp $ */
...
void zmumps_c(ZMUMPS_STRUC_C * zmumps_par)
{
	...
	zmumps_par->eltptr=0; zmumps_par->eltvar=0; zmumps_par->a_elt=0; zmumps_par->perm_in=0; zmumps_par->sym_perm=0; zmumps_par->uns_perm=0; zmumps_par->irn_loc=0;zmumps_par->jcn_loc=0;zmumps_par->a_loc=0; zmumps_par->listvar_schur=0;zmumps_par->schur=0;zmumps_par->mapping=0;zmumps_par->nullspace=0;zmumps_par->colsca=0;zmumps_par->rowsca=0;

	/* Next line initializes scalars to arbitrary values.*/
	zmumps_par->n=0; zmumps_par->nz=0; zmumps_par->nz_loc=0; zmumps_par->nelt=0;zmumps_par->instance_number=0;zmumps_par->deficiency=0;zmumps_par->size_schur=0;
      }
