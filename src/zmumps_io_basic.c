...
int _zmumps_next_file(){
  ...
  mkstemp(name);
#else
  sprintf(name,"%s_%d",zmumps_ooc_file_prefix,zmumps_io_current_file_number+1);
#endif
  if (zmumps_io_current_file_number >= zmumps_io_nb_file-1){
...
#endif
    if(zmumps_io_pfile_pointer_array==NULL){
  ...
  *(zmumps_io_pfile_name+zmumps_io_current_file_number)=(char *)malloc((strlen(name)+1)*sizeof(char));
  ...
  strcpy(*(zmumps_io_pfile_name+zmumps_io_current_file_number),name);
  /* See zmumps_io_basic.h for comments on the I/O flags passed to open */
#ifndef _WIN32
  if(zmumps_io_flag_async){
    *(zmumps_io_pfile_pointer_array+zmumps_io_current_file_number)=open(name,ZMUMPS_IO_FLAGS);
  }else{
    *(zmumps_io_pfile_pointer_array+zmumps_io_current_file_number)=open(name,O_RDWR);
  }
  if(*(zmumps_io_pfile_pointer_array+zmumps_io_current_file_number)==-1){
...
#endif
  zmumps_io_current_file=(zmumps_io_pfile_pointer_array+zmumps_io_current_file_number);
  
  zmumps_io_current_file=(zmumps_io_pfile_pointer_array+zmumps_io_current_file_number);
  zmumps_io_last_file_opened++;
  /*  if(*(zmumps_io_pfile_pointer_array+zmumps_io_current_file_number)==NULL){ */
  zmumps_io_write_pos=0;
  return 0;
}
...
int _zmumps_compute_where_to_write(const double to_be_written){
  ...
  double size;
  int ret_code;
  _zmumps_update_current_file_position();
  size=(double)MAX_FILE_SIZE-((double)zmumps_io_current_file_position+to_be_written);
  if (size > 0){
  }else{
    ret_code=_zmumps_next_file();
    if(ret_code<0){
      return ret_code;
    }
    _zmumps_update_current_file_position();
...
int _zmumps_prepare_pointers_for_write(double to_be_written,int * pos_in_file, int * file_number ){
  int ret_code;
  ret_code=_zmumps_compute_where_to_write(to_be_written);
  if(ret_code<0){
    return ret_code;
  }
  *pos_in_file=zmumps_io_current_file_position;
  *file_number=zmumps_io_current_file_number;
  return 0;
}
...
int zmumps_io_do_write_block(void * address_block,
  ...
  for(i=0;i<nb_concerned_files;i++){
...
#endif
#endif
    ret_code=_zmumps_prepare_pointers_for_write(to_be_written,&pos_in_file_loc,&file_number_loc);
	...
	*file_number=-file_number_loc;
    ...
    if((double)(MAX_FILE_SIZE-zmumps_io_write_pos)>to_be_written){
      write_size=(size_t)to_be_written;
    }else{
      write_size=(size_t)((double)(MAX_FILE_SIZE-zmumps_io_write_pos));
    }
#if defined (_WIN32)
    write_size=(size_t)(int)((write_size)/zmumps_elementary_data_size);
#endif
    file=zmumps_io_current_file;
    where=zmumps_io_write_pos;
#ifndef _WIN32
#ifndef WITHOUT_PFUNC
    if(zmumps_io_flag_async==IO_ASYNC_TH){
      zmumps_io_unprotect_pointers();
  ...
  /*in this case all the I/O's are made by the I/O thread => we don't
...
#else
    ret_code=fwrite(loc_addr,zmumps_elementary_data_size, write_size,*file);
    ...
    loc_addr=loc_addr+((int)write_size*zmumps_elementary_data_size);
#endif
...
int zmumps_io_do_read_block(void * address_block,
  ...
  while(read_size>0){
    file=(zmumps_io_pfile_pointer_array+(local_fnum));
#ifndef _WIN32
    ...
    ret_code=fread(loc_addr,zmumps_elementary_data_size,size,*file);
    if(ret_code!=size){
      sprintf(error_str,"Problem with I/O operation\n");
      return -90;
    }
    ...
    local_fnum++;
    local_offset=0;
    if(local_fnum>zmumps_io_nb_file){
...
int zmumps_free_file_pointers(){
  int i,ierr;
  for(i=0;i<=zmumps_io_last_file_opened;i++){
#ifndef _WIN32    
    ierr=close(*(zmumps_io_pfile_pointer_array+i));
    if(ierr==-1){
      zmumps_io_build_err_str(errno,-90,"Problem while closing OOC file",error_str,200);
      return -90;
    }
#else
    ierr=fclose(*(zmumps_io_pfile_pointer_array+i));
    if(ierr==-1){
      sprintf(error_str,"Problem while opening OOC file\n");
      return -90;
    }    
  ...
  free(zmumps_io_pfile_pointer_array);
  free(zmumps_ooc_file_prefix);
  return 0;
}
...
int zmumps_init_file_structure(int* _myid, int* total_size_io,int* size_element){
  ...
  ierr=zmumps_io_alloc_file_struct(&zmumps_io_nb_file);
  if(ierr<0){
    return ierr;
  }
  /* Init the current file.*/
  ierr=_zmumps_next_file();
  if(ierr<0){
    return ierr;
  }
  return 0;
...
int zmumps_io_alloc_file_struct(int* nb){
#ifndef _WIN32  
  zmumps_io_pfile_pointer_array=(int *)malloc((*nb)*sizeof(int));
  if(zmumps_io_pfile_pointer_array==NULL){
    sprintf(error_str,"Allocation problem in low-level OOC layer\n");
    return -13;
  }
#else  
  zmumps_io_pfile_pointer_array=(FILE **)malloc((*nb)*sizeof(FILE*));
  if(zmumps_io_pfile_pointer_array==NULL){
    sprintf(error_str,"Allocation problem in low-level OOC layer\n");
    return -13;
  }
  ...
  zmumps_io_current_file_number = -1;
  zmumps_io_last_file_opened=-1;
  return 0;
}

int zmumps_io_get_nb_files(int* nb_files){
  *nb_files=zmumps_io_last_file_opened+1;
  return 0;
}
...
int zmumps_io_open_files_for_read(){
  int i;
  for(i=0;i<zmumps_io_nb_file;i++){
#ifndef _WIN32
    if(zmumps_io_flag_async){
      *(zmumps_io_pfile_pointer_array+i)=open(*(zmumps_io_pfile_name+i),ZMUMPS_IO_FLAGS);
    }else{
      *(zmumps_io_pfile_pointer_array+i)=open(*(zmumps_io_pfile_name+i),O_RDWR);
    }
    if(*(zmumps_io_pfile_pointer_array+i)==-1){
...
int zmumps_io_set_last_file(int* dim){
  zmumps_io_last_file_opened=*dim-1;
  return 0;
}
